...
public final class BackgroundExecutor {
     ...
     */
    public static final BackgroundExecutor.WrongThreadListener DEFAULT_WRONG_THREAD_LISTENER = new BackgroundExecutor.WrongThreadListener() {
        @Override
    ...
    private static BackgroundExecutor.WrongThreadListener wrongThreadListener = DEFAULT_WRONG_THREAD_LISTENER;
    ...
    public static void execute(final Runnable runnable, String id, long delay, String serial) {
        execute(new BackgroundExecutor.Task(id, delay, serial) {
            @Override
            public void execute() {
                runnable.run();
            }
    ...
    /**
     * Changes the default {@link BackgroundExecutor.WrongThreadListener}. To restore the default
     * one use {@link #DEFAULT_WRONG_THREAD_LISTENER}.
     *
     * @param listener the new {@link BackgroundExecutor.WrongThreadListener}
     */
    ...
    public static synchronized void cancelAll(String id, boolean mayInterruptIfRunning) {
        for (int i = TASKS.size() - 1; i >= 0; i--) {
            BackgroundExecutor.Task task = TASKS.get(i);
     ...
     * {@link BackgroundExecutor.WrongThreadListener#onBgExpected(String...)} will be called. If
     * the current thread is not UI and serials list is empty, then this method
     * just returns. Otherwise, if the method was called not during {@link BackgroundExecutor.Task}
     * execution or the task has no serial, then the
     * {@link BackgroundExecutor.WrongThreadListener#onWrongBgSerial(String, String...)} will be
    ...
    public static abstract class Task implements Runnable {
        ...
        private void postExecute() {
            ...
            synchronized (BackgroundExecutor.class) {
                ...
                if (serial != null) {
                    BackgroundExecutor.Task next = take(serial);
                    if (next != null) {
